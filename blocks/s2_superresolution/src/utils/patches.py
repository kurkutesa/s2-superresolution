...
def interp_patches(image_20, image_10_shape) -> np.ndarray:
    ...
    for k_i in range(image_20.shape[0]):
        for w_i in range(image_20.shape[1]):
            data20_interp[k_i, w_i] = resize(image_20[k_i, w_i] / 30000,
                                             image_10_shape[2:4],
                                             mode='reflect') * 30000  # bilinear
...
def get_test_patches(dset_10, dset_20, patchsize=128, border=4, interp=True) -> Tuple:
    ...
    dset_10 = np.pad(dset_10, ((border_hr, border_hr),
                               (border_hr, border_hr), (0, 0)), mode='symmetric')
    dset_20 = np.pad(dset_20, ((border_lr, border_lr),
                               (border_lr, border_lr), (0, 0)), mode='symmetric')
    ...
    label_20 = np.zeros((nr_patches, bands20) + patch_size_hr).astype(np.float32) # pylint: disable=unused-variable
    ...
    range_i = np.arange(0, (dset_20.shape[0] - 2 * border_lr) //
                        (patch_size_lr[0] - 2 * border_lr)) * (patch_size_lr[0] - 2 * border_lr)
    range_j = np.arange(0, (dset_20.shape[1] - 2 * border_lr) //
                        (patch_size_lr[1] - 2 * border_lr)) * (patch_size_lr[1] - 2 * border_lr)
    ...
    for i_r in range_i.astype(int):
        for j_r in range_j.astype(int):
            ...
            image_20[p_count] = np.rollaxis(dset_20[crop_point_lr[0]:crop_point_lr[2],
                                                    crop_point_lr[1]:crop_point_lr[3]], 2)
            image_10[p_count] = np.rollaxis(dset_10[crop_point_hr[0]:crop_point_hr[2],
...
def get_test_patches60(dset_10, dset_20, dset_60, patchsize=128, border=8, interp=True) -> Tuple:
    ...
    # Mirror the data at the borders to have the same dimensions as the input
    dset_10 = np.pad(dset_10, ((border_10, border_10),
                               (border_10, border_10), (0, 0)), mode='symmetric')
    dset_20 = np.pad(dset_20, ((border_20, border_20),
                               (border_20, border_20), (0, 0)), mode='symmetric')
    dset_60 = np.pad(dset_60, ((border_60, border_60),
                               (border_60, border_60), (0, 0)), mode='symmetric')
    ...
    range_i = np.arange(0, (dset_60.shape[0] - 2 * border_60) //
                        (patch_size_60[0] - 2 * border_60)) * (patch_size_60[0] - 2 * border_60)
    range_j = np.arange(0, (dset_60.shape[1] - 2 * border_60) //
                        (patch_size_60[1] - 2 * border_60)) * (patch_size_60[1] - 2 * border_60)
    ...
    for i_r in range_i.astype(int):
        for j_r in range_j.astype(int):
            ...
            image_10[p_count] = np.rollaxis(dset_10[crop_point_10[0]:crop_point_10[2],
                                                    crop_point_10[1]:crop_point_10[3]], 2)
            image_20[p_count] = np.rollaxis(dset_20[crop_point_20[0]:crop_point_20[2],
                                                    crop_point_20[1]:crop_point_20[3]], 2)
            image_60[p_count] = np.rollaxis(dset_60[crop_point_60[0]:crop_point_60[2],
...
def save_random_patches(dset_20gt, dset_10, dset_20, file, nr_crop=8000):
    ...
    for crop in range(0, nr_crop): # pylint: disable=unused-variable
        ...
        crop_point_hr = [p*2 for p in crop_point_lr]
        label_20[i] = np.rollaxis(dset_20gt[crop_point_hr[0]:crop_point_hr[2],
                                            crop_point_hr[1]:crop_point_hr[3]], 2)
        image_20[i] = np.rollaxis(dset_20[crop_point_lr[0]:crop_point_lr[2],
                                          crop_point_lr[1]:crop_point_lr[3]], 2)
        image_10[i] = np.rollaxis(dset_10[crop_point_hr[0]:crop_point_hr[2],
                                          crop_point_hr[1]:crop_point_hr[3]], 2)
        i += 1
    np.save(file + 'data10', image_10)
    image_10_shape = image_10.shape
...
def save_random_patches60(dset_60gt, dset_10, dset_20, dset_60, file, nr_crop=500):
    ...
    for crop in range(0, nr_crop): # pylint: disable=unused-variable
        ...
        label_60[i] = np.rollaxis(dset_60gt[crop_point_hr60[0]:crop_point_hr60[2],
                                            crop_point_hr60[1]:crop_point_hr60[3]], 2)
        image_10[i] = np.rollaxis(dset_10[crop_point_hr60[0]:crop_point_hr60[2],
                                          crop_point_hr60[1]:crop_point_hr60[3]], 2)
        image_20[i] = np.rollaxis(dset_20[crop_point_hr20[0]:crop_point_hr20[2],
                                          crop_point_hr20[1]:crop_point_hr20[3]], 2)
        image_60[i] = np.rollaxis(dset_60[crop_point_lr[0]:crop_point_lr[2],
                                          crop_point_lr[1]:crop_point_lr[3]], 2)
        i += 1
    np.save(file + 'data10', image_10)
    image_10_shape = image_10.shape
...
def opendatafiles(path, run_60, scale):
    ...
    for dset in file_list:
        ...
        data10 = np.concatenate((data10, data10_new)) if\
            data10 is not None else data10_new
        data20 = np.concatenate((data20, data20_new)) if\
            data20 is not None else data20_new
        if run_60:
            data60_gt_new = np.load(train_path + dset + '/data60_gt.npy')
            data60_new = np.load(train_path + dset + '/data60.npy')
            data60_gt = np.concatenate((data60_gt, data60_gt_new)) if\
                data60_gt is not None else data60_gt_new
            data60 = np.concatenate((data60, data60_new)) if\
                data60 is not None else data60_new
        else:
            data20_gt_new = np.load(train_path + dset + '/data20_gt.npy')
            data20_gt = np.concatenate((data20_gt, data20_gt_new)) if\
                data20_gt is not None else data20_gt_new
